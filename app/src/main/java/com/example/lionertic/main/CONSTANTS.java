package com.example.lionertic.main;

public class CONSTANTS {

    private static final String ROOT_URL = "http://mapsu.serveo.net/Server/";

    public static final String INSERT = ROOT_URL+"Driver/update.php";

    public static final String CHECK= ROOT_URL+"Driver/check.php";

    public static final String REGISTER = ROOT_URL+"Driver/register.php";

    public static final String LOGIN = ROOT_URL+"Driver/login.php";

    public static final String ROUTE = ROOT_URL+"Service/driverRoute.php";

}

